...
def rebuild_project(cfg='Release'):
...
def project_is_installed(projname):
    project_loader = False
    try:
        import pkgutil
        project_loader = pkgutil.find_loader(projname)
    except ImportError:
        import importlib
        project_loader = importlib.util.find_spec(projname)
    return project_loader is not None
...
def remove_installed_project(projname):
    if project_is_installed():
        os.system('echo y | ' + sys.executable +
                  ' -m pip uninstall ' + projname + '')
        assert is_devel_install() or not project_is_installed()
        print('uninstalled ' + projname + ' from ' + sys.executable)
        sys.stdout.flush()
...
def rebuild_fast(target, cfg='Release', force_redo_cmake=False):
    try:
        if force_redo_cmake:
            raise OSError
        ...
        return rebuild_project(cfg=cfg)


def build_and_test(projname):
    ...
    apps = [x for x in argfiles if '/' + projname + '/apps/' in x]
    tests = [x for x in argfiles if '/' + projname + '/apps/' not in x]
    ...
    shutil.copy(join(srcdir, projname, 'conftest.py'),
                join(libdir, projname, 'conftest.py'))
    ...
    if tests or not apps:
        ...
        if _VERBOSE:
            ...
            print(args)
            sys.stdout.write('    args:' + os.linesep + '       ')
            for arg in args:
                if arg.startswith('-'):
...
def build_and_run_gtest_auto(projname):
